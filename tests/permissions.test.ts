...
import { canCreateForm, canEditForm, canViewForm } from "../lib/permissions";

test("sub-admins retain the original quote creation and editing workflow", () => {
  assert.equal(canViewForm("sub_admin", "QUOTE"), true);
...
test("sub-admins cannot create, edit, or convert records into invoices", () => {
  assert.equal(canViewForm("sub_admin", "FORM"), false);
...
test("admins can create and edit both document types", () => {
  assert.equal(canViewForm("admin", "FORM"), true);
  assert.equal(canViewForm("admin", "QUOTE"), true);
